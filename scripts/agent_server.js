...
function sendSafe(ws, data) {
  try {
    ...
    console.warn('[Server] send failed:', e && e.message);
  }
}
  ...
  ws.on('message', (raw) => {
    ...
    if (data.type === 'auth') {
      ...
      if (!agent || !verifyChallenge(challenge, response, agent.accessKey)) {
        sendSafe(ws, { type: 'auth_response', status: 'failure', message: 'Invalid credentials' });
        return;
      }
      agentId = id;
      ...
      sendSafe(ws, { type: 'auth_response', status: 'success', sessionId: crypto.randomUUID(), role });
